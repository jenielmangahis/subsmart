...
function DELETE_ALL() {
    ...
    $delete = $db->executeQuery("delete from work_orders");
...
function DELETE($id) {
    ...
    $db = new database_handler();
    // get signature
    $rows = $db->fetchAll("select * from work_orders where company_id = $id");
    $before =  "/home4/admintommy/public_html/".$rows[0]['before_signature'];
    $after =  "/home4/admintommy/public_html/".$rows[0]['after_signature'];
    $owner =  "/home4/admintommy/public_html/".$rows[0]['owner_signature'];

    // delete
    $delete = $db->executeQuery("delete from work_orders where id = $id");
    ...
    if($delete) {
        ...
        // delete items
        $delete2 = $db->executeQuery("delete from work_orders_items where work_order_id = $id");

        // get photos
        $photos = $db->fetchAll("select * from work_orders_photo where work_order_id = $id");
        // iterate
        ...
        // delete photos
        $delete3 = $db->executeQuery("delete from work_orders_photo where work_order_id = $id");
...
function INSERT() {
    ...
    $db = new database_handler();
    $insert = $db->insertQuery($params, "work_orders");
    ...
    if($insert) {
        ...
        // get invoice number
        $number = explode("-", $params['work_order_number']);
        // update workorder number
        $workOrder_settings['work_order_num_next'] = (int)$number[1] + 1;
        $update = $db->updateQuery($workOrder_settings, 'work_order_settings', $company_id,'company_id');
...
function UPDATE($id, $deleteItems = "") {
    ...
    $db = new database_handler();
    $update = $db->updateQuery($params,'work_orders', $id,'id');
