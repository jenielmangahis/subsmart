...
function DELETE($id) {
    ...
    $row = $db->fetchRow("select * from estimates_photo where id = $id");
    $path = "/home4/admintommy/public_html/".$row['path'];

    // delete
    $delete = $db->executeQuery("delete from estimates_photo where id = $id");
...
function INSERT() {
    ...
    $params['estimate_id']  = $_POST['estimate_id'];
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_dir)) {
        // save to db
        $db = new database_handler();
        $insert = $db->insertQuery($params, "estimates_photo");
