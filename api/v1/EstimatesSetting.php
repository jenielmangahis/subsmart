...
function GET($id) {
    ...
    $row = $db->fetchRow("select * from estimate_settings where company_id = $id");
    ...
    $row['hide_grand_total']            = (bool)$row['hide_grand_total'];
...
function UPDATE($id) {
    ...
    // check first
    $db = new database_handler();
    $row = $db->fetchRow("select count(*) as count from estimate_settings where company_id = $id");
    //
    if ($row['count'] == 1) {
        // update
        $update = $db->updateQuery($params,'estimate_settings', $id,'company_id');
    } else {
        // insert
        $insert = $db->insertQuery($params, "estimate_settings");
    }
