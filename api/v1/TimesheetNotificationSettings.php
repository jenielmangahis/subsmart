...
function GET($id) {
    ...
    $row = $db->fetchRow("select * from timesheet_notification_settings where company_id = $id");
...
function UPDATE($id) {
    ...
    // check first
    $db = new database_handler();
    $row = $db->fetchRow("select count(*) as count from timesheet_notification_settings where company_id = $id");
    //
    if ($row['count'] == 1) {
        // update
        $update = $db->updateQuery($params,'timesheet_notification_settings', $id,'company_id');
    } else {
        // insert
        $insert = $db->insertQuery($params, "timesheet_notification_settings");
    }
