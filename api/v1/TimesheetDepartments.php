...
function DELETE_ALL() {
    ...
    $delete = $db->executeQuery("delete from timesheet_departments");
...
function DELETE($id) {
    ...
    $db = new database_handler();
    $delete = $db->executeQuery("delete from timesheet_departments where id = $id");
...
function GET($id, $flag = "ALL") {
    // curl
    $db = new database_handler();
    $rows = $db->fetchAll("select * from timesheet_departments where company_id = $id order by id asc");

    if ($flag == "ONE") {
        $rows = $db->fetchAll("select * from timesheet_departments where id = $id");
    }
    ...
    foreach ($rows as $row) {
        ...
        // get members
        $members = $db->fetchAll("select * from timesheet_team_members where department_id = $id");
        $row['members'] = $members;
...
function INSERT() {
    ...
    $db = new database_handler();
    $insert = $db->insertQuery($params, "timesheet_departments");
...
function UPDATE($id) {
    ...
    $db = new database_handler();
    $update = $db->updateQuery($params,'timesheet_departments', $id,'id');
