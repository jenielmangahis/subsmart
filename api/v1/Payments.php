...
function DELETE_ALL() {
    ...
    $delete = $db->executeQuery("delete from payments");
...
function DELETE($id) {
    ...
    $db = new database_handler();
    $delete = $db->executeQuery("delete from payments where id = $id");
...
function GET($id, $flag = "ALL") {
    // curl
    $db = new database_handler();
    $rows = $db->fetchAll("select * from payments where company_id = $id");

    if ($flag == "ONE") {
        $rows = $db->fetchAll("select * from payments where invoice_id = $id");
    }
...
function INSERT() {
    ...
    $db = new database_handler();
    $insert = $db->insertQuery($params, "payments");
...
function UPDATE($id) {
    ...
    $db = new database_handler();
    $update = $db->updateQuery($params,'payments', $id,'id');
