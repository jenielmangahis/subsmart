...
function DELETE_ALL() {
    ...
    $delete = $db->executeQuery("delete from timesheet_attendance");
...
function DELETE($id) {
    ...
    $db = new database_handler();
    $delete = $db->executeQuery("delete from timesheet_attendance where id = $id");

    if($delete) {
        // delete logs
        $delete2 = $db->executeQuery("delete from timesheet_logs where attendance_id = $id");
...
function GET($id, $flag = "ALL") {
    // curl
    $db = new database_handler();
    $rows = $db->fetchAll("select * from timesheet_attendance where user_id = $id order by id asc");

    if ($flag == "ONE") {
        $rows = $db->fetchAll("select * from timesheet_attendance where id = $id");
    }
...
function INSERT() {
    ...
    $db = new database_handler();
    $insert = $db->insertQuery($params, "timesheet_attendance");
