...
const columns = {
  title: (_, __, row) => {
    return `<a href="#" class="link nsm-link" data-action="preview">${row.title}</a>`;
  },
  category: (_, __, row) => {
    return row.category;
  },
...
const actions = {
  edit: async (row) => {
    window.location.href = `${window.api.prefixURL}/EsignEditor_v2/edit?id=${row.id}`;
  },
  ...
  duplicate: async (row) => {
    ...
    window.location.href = `${window.api.prefixURL}/EsignEditor_v2/edit?id=${data.id}`;
  },
};
