...
$(document).on('change', '#pay-schedule', function() {
    ...
    if(el.val() === 'add') {
        ...
        $('#pay-schedule-modal #edit-pay-schedule-form').attr('id', 'add-pay-schedule-form');
        $('#edit-employment-details-modal').modal('hide');
        ...
        $.get('/accounting/employees/get-pay-date/'+el.val(), function(res) {
            ...
            $('#edit-employment-details-modal label[for="pay-schedule"] span').html(result.date);
        });
    }
});
    ...
    if($('#pay-schedule-modal #pay-frequency').val() === 'twice-month' || $('#pay-schedule-modal #pay-frequency').val() === 'every-month') {
        if($(this).prop('checked')) {
            ...
            if($('#pay-schedule-modal #next-payday').length < 1) {
                ...
                $('#next-payday, #next-pay-period-end').datepicker({
                    startDate: new Date(),
                    format: 'mm/dd/yyyy',
                    orientation: 'bottom',
                    autoclose: true
...
$(document).on('change', '#pay-schedule-modal #next-payday, #pay-schedule-modal #next-pay-period-end', function() {
    ...
    if($(this).attr('id') === 'next-payday') {
        switch($('#pay-schedule-modal #pay-frequency').val()) {
            ...
            break;
            default :
                $(`#pay-schedule-modal #name`).val($('#pay-schedule-modal #pay-frequency option:selected').html());
            break;
        }
    }
...
$(document).on('hidden.bs.modal', '#pay-schedule-modal', function(e) {
    if($('#edit-employment-details-modal #pay-schedule').val() === 'add') {
        $('#edit-employment-details-modal #pay-schedule option').each(function() {
            if (typeof $(this).attr('selected') !== 'undefined' && $(this).attr('selected') !== false) {
                $('#edit-employment-details-modal #pay-schedule').val($(this).val()).trigger('change');
            }
        });
    }
...
$(document).on('submit', '#add-pay-schedule-form', function(e) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            $('#edit-employment-details-modal #pay-schedule option:selected').removeAttr('selected');
            $('#edit-employment-details-modal #pay-schedule').append(`<option value="${result.id}" selected>${result.name}</option>`);
            $('#edit-employment-details-modal #pay-schedule').trigger('change');
...
$(document).on('click', '#edit-employment-details-modal #edit-pay-schedule', function(e) {
    e.preventDefault();
    var pay_sched_id = $('#edit-employment-details-modal #pay-schedule').val();
...
$(document).on('submit', '#edit-pay-schedule-form', function(e) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            var prevOpt = $('#edit-employment-details-modal #pay-schedule option:selected').prev();
            $('#edit-employment-details-modal #pay-schedule option:selected').remove();
