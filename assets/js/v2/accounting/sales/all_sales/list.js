...
$('.dropdown-menu.table-settings input[name="col_chk"]').on(
  "change",
  function () {
    var chk = $(this);
    ...
    var index = $(
      `#transactions-table thead td[data-name="${dataName}"]`
    ).index();
    ...
    $(
      `#print_preview_all_sales_transactions_modal #all_sales_transactions_table_print tr`
    ).each(function () {
      if (chk.prop("checked")) {
        $($(this).find("td")[index - 1]).show();
      } else {
...
$(".dropdown-menu.table-settings, .dropdown-menu.p-3").on(
  "click",
  function (e) {
    e.stopPropagation();
  }
...
$("#apply-button").on("click", function () {
  const noDate = ["unbilled-income", "recently-paid"];
  ...
  url +=
    filterStatus.length > 0 && filterStatus.val() !== "all-statuses"
      ? `status=${filterStatus.val()}&`
      : "";
  url +=
    filterDeliveryMethod.length > 0 && filterDeliveryMethod.val() !== "any"
      ? `delivery-method=${filterDeliveryMethod.val()}&`
      : "";
  url +=
    noDate.includes(filterType) === false && filterDate !== "last-365-days"
      ? `date=${filterDate}&`
      : "";
  url +=
    filterType === "unbilled-income"
      ? `date=${filterAsOf.replaceAll("/", "-")}&`
      : "";
  url +=
    filterType !== "unbilled-income" &&
    filterType !== "recently-paid" &&
    filterDate !== "last-365-days"
      ? `from=${filterFrom.replaceAll("/", "-")}&`
      : "";
  url +=
    filterType !== "unbilled-income" &&
    filterType !== "recently-paid" &&
    filterDate !== "last-365-days"
      ? `to=${filterTo.replaceAll("/", "-")}&`
      : "";
  url +=
    filterType !== "unbilled-income" && filterCustomer !== "all"
...
var date_filter_select = `<div class="row">
<div class="col-4">
...
$(document).on('input', '.select2-search__field', function() {
    console.log('User is inputting');
     var dropdownMenu = document.getElementById('filter_dropdown_body');
     document.getElementById('filter_button').classList.add('show');
    dropdownMenu.style.width = 'max-content';
    dropdownMenu.style.position = 'absolute';
    dropdownMenu.style.inset = '0px auto auto 0px';
    dropdownMenu.style.margin = '0px';
    dropdownMenu.style.transform = 'translate(-602px, 41px)';
    dropdownMenu.classList.add('show');
    // Add your logic here for when the user is inputting text
});

$(document).on('click', '.select2-search__field', function() {
  console.log('User is click');
   var dropdownMenu = document.getElementById('filter_dropdown_body');
   document.getElementById('filter_button').classList.add('show');
  dropdownMenu.style.width = 'max-content';
  dropdownMenu.style.position = 'absolute';
...
$(document).on('click', '.select2-results', function() {
  console.log('User is clicking select');
   var dropdownMenu = document.getElementById('filter_dropdown_body');
   document.getElementById('filter_button').classList.add('show');
  dropdownMenu.style.width = 'max-content';
  dropdownMenu.style.position = 'absolute';
  ...
  switch ($(this).val()) {
    ...
    $("#delivery-filter-container").removeClass("col-12").addClass("col-6");
      if ($("#filter-date").length < 1) {
        ...
        $("#filter-status")
          .html(`<option value="all-statuses" selected>All statuses</option>
                <option value="open">Open</option>
                <option value="closed">Closed</option>
                <option value="draft">Draft</option>
                <option value="submitted">Submitted</option>
...
$("#new-time-activity").on("click", function () {
  $.get(
    "/accounting/get-other-modals/single_time_activity_modal",
    function (res) {
      if ($("div#modal-container").length > 0) {
        $("div#modal-container").html(res);
      } else {
...
$(document).on(
  "click",
  "#transactions-table .view-edit-time-charge",
  function (e) {
    e.preventDefault();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-billable-expense",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
    ...
    $.get(
      "/accounting/view-transaction/billable-expense/" + id,
      function (res) {
        if ($("div#modal-container").length > 0) {
          $("div#modal-container").html(res);
        } else {
...
$(document).on(
  "click",
  "#transactions-table .view-edit-estimate",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-credit-memo",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-sales-receipt",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-refund-receipt",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-delayed-credit",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$(document).on(
  "click",
  "#transactions-table .view-edit-delayed-charge",
  function (e) {
    e.preventDefault();
    var row = $(this).closest("tr");
    var id = row.find(".select-one").val();
...
$("#transactions-table .copy-transaction").on("click", function (e) {
  ...
  $.get(
    `/accounting/copy-transaction/${transactionType}/${id}`,
    function (res) {
      if ($("div#modal-container").length > 0) {
        $("div#modal-container").html(res);
      } else {
...
$("#transactions-table .create-invoice").on("click", function (e) {
  ...
  $.get(
    `/accounting/customers/create-invoice/${type
      .toLowerCase()
      .replaceAll(" ", "-")}/${id}`,
    function (res) {
      if ($("div#modal-container").length > 0) {
...
$("#transactions-table .send-estimate").on("click", function (e) {
  e.preventDefault();
  var refnumIndex = $(
    '#transactions-table thead tr td[data-name="No."]'
  ).index();
  var emailIndex = $(
    '#transactions-table thead tr td[data-name="Email"]'
  ).index();
  var customerIndex = $(
    '#transactions-table thead tr td[data-name="Customer"]'
  ).index();
  ...
  $("#send-transaction-email #email-subject").val(
    `Estimate ${ref_no} from ${companyName}`
  );
...
$(document).on(
  "click",
  "#transactions-table .update-estimate-status",
  function (e) {
    e.preventDefault();
    var estimateId = $(this).closest("tr").find(".select-one").val();
    var url = `/accounting/customers/update-estimate-status/${estimateId}`;

    $("#update-status-modal #update-estimate-status-form")
      .attr("action", url)
      .attr("method", "post");
    $("#update-status-modal #status")
      .val($(this).closest("tr").find("td:nth-child(16)").text().trim())
      .trigger("change");
...
$("#transactions-table .send-invoice").on("click", function (e) {
  ...
  $("#send-transaction-email #email-to").val(email);
  $("#send-transaction-email #email-subject").val(
    `New payment request from ${companyName} - invoice ${ref_no}`
  );
  $("#send-transaction-email #email-message").val(`Dear ${customerName.trim()},
...
$("#transactions-table .send-sales-receipt").on("click", function (e) {
  ...
  $("#send-transaction-email #email-to").val(email);
  $("#send-transaction-email #email-subject").val(
    `Sales Receipt #${ref_no} from ${companyName}`
  );
  $("#send-transaction-email #email-message").val(`Dear ${customerName.trim()},
...
$("#transactions-table .send-refund-receipt").on("click", function (e) {
  ...
  $("#send-transaction-email #email-to").val(email);
  $("#send-transaction-email #email-subject").val(
    `Refund Receipt from ${companyName}`
  );
  $("#send-transaction-email #email-message").val(`Dear ${customerName.trim()},
...
$(".export-transactions").on("click", function () {
  if ($("#export-form").length < 1) {
    $("body").append(
      `<form action="/accounting/all-sales/export" method="post" id="export-form"></form>`
    );
  }

  var fields = $(
    "#transactions-table thead tr td:not(:first-child, :last-child)"
  );
  fields.each(function () {
    $("#export-form").append(
      `<input type="hidden" name="fields[]" value="${$(this).attr(
        "data-name"
      )}">`
    );
  });
  ...
  if (query !== undefined) {
    ...
    $.each(querySplit, function (key, value) {
      ...
      $("#export-form").append(
        `<input type="hidden" name="${selectedVal[0]}" value="${selectedVal[1]}">`
      );
    });
  }
...
$(document).on("change", "#transactions-table thead .select-all", function () {
  $("#transactions-table tbody tr:visible .select-one").prop(
    "checked",
    $(this).prop("checked")
  );
  setTimeout(function () {
...
function handleTriggerChange() {
  $("#transactions-table tbody tr:visible .select-one").trigger("change");
}

$(document).on("change", "#transactions-table tbody .select-one", function () {
  var checked = $(
    "#transactions-table tbody tr:visible .select-one:checked"
  ).length;
  ...
  var invoiceOpenNotStatus = ["Draft", "Declined", "Paid"];

  $("#transactions-table tbody tr:visible .select-one:checked").each(
    function () {
      var statusIndex = $(
        '#transactions-table thead tr td[data-name="Status"]'
      ).index();
      var row = $(this).closest("tr");
      ...
      if (
        (type === "Invoice" && invoiceOpenNotStatus.includes(status)) ||
        type !== "Invoice"
      ) {
...
$("#send-transactions").on("click", function (e) {
  ...
  $("#transactions-table tbody .select-one:checked").each(function () {
    ...
    var id = $(this).val();
    var typeIndex = $(
      '#transactions-table thead tr td[data-name="Type"]'
    ).index();
...
$("#print-transactions").on("click", function (e) {
  if ($("#print-transactions-form").length < 1) {
    $("body").append(
      `<form action="/accounting/print-sales-transactions" method="post" id="print-transactions-form" target="_blank"></form>`
    );
  }
  ...
  $("#transactions-table tbody .select-one:checked").each(function () {
    ...
    $("#print-transactions-form").append(
      `<input type="hidden" name="transactions[]" value="${type.replaceAll(
        " ",
        "_"
      )}-${id}">`
    );
