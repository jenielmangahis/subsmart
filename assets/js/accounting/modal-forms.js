...
$(function () {
    ...
    $(document).on('click', '#statementModal div.modal-footer button#save-and-send', function (e) {
        ...
        if (flag === true) {
            $.ajax({
                url: base_url + 'accounting/send-email-form/',
    ...
    $(document).on('submit', '#vendor-modal #ajax-add-vendor', function (e) {
        e.preventDefault();
        var is_valid = 1;
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.is_success == 1) {
                        dropdownEl.append(`<option value="${result.vendor_id}" selected>${result.vendor_name}</option>`);
                        dropdownEl.trigger('change');
    ...
    $(document).on('click', '#modal-container #item_list table button', function (e) {
        ...
        $.get(base_url + 'accounting/get-item-details/' + id, function (res) {
            ...
            var fields = `
                ...
                <td>${type.charAt(0).toUpperCase() + type.slice(1)}</td>
                <td>${locs}</td>
                <td><input type="number" name="quantity[]" class="form-control nsm-field text-end" required value="0" min="1" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"></td>
                <td><input type="number" name="item_amount[]" onchange="convertToDecimal(this)" class="form-control nsm-field text-end" step="any" value="${item.price}"></td>
                <td><input type="number" name="discount[]" onchange="convertToDecimal(this)" class="form-control nsm-field text-end" step="any" value="0.00"></td>
                <td><input type="number" name="item_tax[]" onchange="convertToDecimal(this)" class="form-control nsm-field text-end" step="any" value="7.50"></td>
...
const clearForm = () => {
    ...
    if (modalName === '#weeklyTimesheetModal') {
        ...
        $(modalName).find('.modal-body table tbody tr').each(function (index, value) {
            ...
            if (index < rowCount) {
                if (table !== 'category-details-table' && table !== 'item-details-table' && table !== 'timesheet-table' && table !== 'journal-table') {
                    $(this).html(blankRow);
                } else {
                    if (table === 'category-details-table') {
                        $(this).html(catDetailsBlank);
