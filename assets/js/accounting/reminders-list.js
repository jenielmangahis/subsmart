...
$('#table_rows').on('change', function() {
    $('#reminders-list').DataTable().ajax.reload();
});
...
$('#transaction_type_modal .modal-footer .btn-success').on('click', function(e) {
    ...
    $.get(GET_OTHER_MODAL_URL+view, function(res) {
        ...
        $(`#${modalName} #recurringType`).val('reminder').change();
        ...
        fnCreatedCell: function(td, cellData, rowData, row, col) {
            ...
            <div class="d-flex justify-content-center">
                <div class="checkbox checkbox-sec m-0">
                    <input type="checkbox" value="${rowData.id}" id="reminder-${rowData.id}">
                    <label for="reminder-${rowData.id}" class="p-0" style="width: 24px; height: 24px"></label>
                </div>
            </div>
            `);
        }
...
var table = $('#reminders-list').DataTable({
    autoWidth: false,
    searching: false,
    processing: true,
    serverSide: true,
    ...
    info: false,
    ajax: {
        url: '/accounting/recurring-transactions/load-reminders-list/',
        dataType: 'json',
...
$(document).on('click', '#reminders-list .delete-recurring', function(e) {
    e.preventDefault();
...
$(document).on('click', '#reminders-list .edit-recurring', function(e) {
    e.preventDefault();
    var row = $(this).parent().parent().parent();
    var rowData = table.row(row).data();
    var modal = '';
...
$(document).on('click', '#reminders-list .use-transaction', function(e) {
    e.preventDefault();
...
$(document).on('click', '#reminders-list .duplicate-transaction', function(e) {
    e.preventDefault();
    var row = $(this).parent().parent().parent().parent();
    var rowData = table.row(row).data();
    var transactionType = '';
...
$(document).on('click', '#reminders-list .skip-next-date', function(e) {
    e.preventDefault();
    var row = $(this).parent().parent().parent().parent();
    var rowData = table.row(row).data();
...
$(document).on('click', '#reminders-list .pause-recurring', function(e) {
    e.preventDefault();
    var row = $(this).parent().parent().parent().parent();
    var rowData = table.row(row).data();
...
$(document).on('click', '#reminders-list .resume-recurring', function(e) {
    e.preventDefault();
    var row = $(this).parent().parent().parent().parent();
    var rowData = table.row(row).data();
...
$(document).on('change', '#reminders-list #select-all-reminders', function() {
    $('#reminders-list tbody input[type="checkbox"]').prop('checked', $(this).prop('checked'));
});

$(document).on('change', '#reminders-list tbody input[type="checkbox"]', function() {
    var checked = $('#reminders-list tbody input[type="checkbox"]:checked').length;
    var totalRow = $('#reminders-list tbody tr').length;

    $('#reminders-list #select-all-reminders').prop('checked', checked === totalRow);

    if(checked > 0) {
        $('#skip-batch.dropdown-item').removeClass('disabled');
        $('#create-batch.dropdown-item').removeClass('disabled');
    } else {
        $('#skip-batch.dropdown-item').addClass('disabled');
        $('#create-batch.dropdown-item').addClass('disabled');
    }
});
...
$('#skip-batch').on('click', function(e) {
    e.preventDefault();

    var data = new FormData();

    $('#reminders-list tbody tr').each(function() {
        var rowData = $('#reminders-list').DataTable().row($(this)).data();

        if($(this).find('input[type="checkbox"]').prop('checked')) {
            if(data.has('transaction_ids[]')) {
                data.append('transaction_ids[]', rowData.id);
            } else {
                data.set('transaction_ids[]', rowData.id);
            }
        }
    });

    $.ajax({
        url: '/accounting/recurring-transactions/skip-batch',
        data: data,
        type: 'post',
        processData: false,
        contentType: false,
        success: function(res) {
            ...
            $.toast({
                icon: result.success ? 'success' : 'error',
                heading: result.success ? 'Success' : 'Error',
                text: result.success ? 'Skip successful' : 'Unexpected error occured',
                showHideTransition: 'fade',
                hideAfter: 3000,
                allowToastClose: true,
                position: 'top-center',
