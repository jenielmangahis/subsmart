...
$('#run-report-button').on('click', function() {
    ...
    url += $('#group-by').val() !== 'employee' ? `group-by=${$('#group-by').val()}&` : '';
    url += $('input[name="total_display"]:checked').val() !== 'total-and-details' ? `total-display=${$('input[name="total_display"]:checked').val()}` : '';
    var columns = [];
...
$('#display-by-row').on('click', function(e) {
    var url = `${base_url}accounting/reports/view-report/${reportId}?`;
    var currentUrl = currUrl.replace('#', '');
    var urlSplit = currentUrl.split('?');
    var query = urlSplit[1];
...
$('#display-by-col').on('click', function(e) {
    var url = `${base_url}accounting/reports/view-report/${reportId}?`;
    var currentUrl = currUrl.replace('#', '');
    var urlSplit = currentUrl.split('?');
    var query = urlSplit[1];
