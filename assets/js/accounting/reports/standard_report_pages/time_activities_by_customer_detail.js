...
$('#filter-customer, #filter-employee, #filter-product-service, #filter-billable').on('change', function() {
    if($(this).val() !== 'all') {
...
$('#run-report-button').on('click', function() {
    ...
    var url = `${base_url}accounting/reports/view-report/${reportId}?`;
    url += filterDate !== 'this-month-to-date' ? `date=${filterDate}&` : '';
    url += filterDate !== 'this-month-to-date' && filterDate !== 'all-dates' ? `from=${$('#time-activity-date-from').val().replaceAll('/', '-')}&to=${$('#time-activity-date-to').val().replaceAll('/', '-')}&` : '';
    url += sortBy !== 'default' ? `column=${sortBy}&` : '';
    url += sortIn !== 'asc' ? `order=${sortIn}&` : '';
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Time Activities by Customer Detail' ? `report-title=${$('#report-title').val()}&` : '';
...
$('input[name="col_chk"]').on('change', function() {
    ...
    if(groupBy !== 'none') {
        ...
        if($($('#reports-table thead tr td:visible:not([data-name=""])')[0]).index() > $('#reports-table thead tr td:visible[data-name="Duration"]').index() && $($('#reports-table thead tr td:visible:not([data-name=""])')[0]).index() < $('#reports-table thead tr td:visible[data-name="Amount"]').index()) {
            var dataName = $($('#reports-table thead tr td:visible:not([data-name=""])')[0]).data().name;
            ...
            $(`#reports-table tbody tr[data-bs-toggle="collapse"] td[data-name="${dataName}"], #reports-table tbody tr.group-total td[data-name="${dataName}"]`).hide();
            $('#reports-table tbody tr[data-bs-toggle="collapse"] td:first-child, #reports-table tbody tr.group-total td:first-child').attr('colspan', 0);
            ...
            $(`#print_report_modal tbody tr.group-header td[data-name="${dataName}"], #print_report_modal tbody tr.group-total td[data-name="${dataName}"]`).hide();
            $('#print_report_modal tbody tr.group-header td:first-child, #print_report_modal tbody tr.group-total td:first-child').attr('colspan', 0);
            ...
            $(`#print_preview_report_modal tbody tr.group-header td[data-name="${dataName}"], #print_preview_report_modal tbody tr.group-total td[data-name="${dataName}"]`).hide();
            $('#print_preview_report_modal tbody tr.group-header td:first-child, #print_preview_report_modal tbody tr.group-total td:first-child').attr('colspan', 0);
        }
        ...
        $($('#reports-table thead tr td:visible')[0]).data().name !== '') {
            ...
            $('#reports-table thead tr td:visible').each(function() {
                ...
                if(name === 'Amount' || name === 'Duration' || inBetweenCols.includes(name)) {
                    return false;
                } else {
                    colspan++;
                }
