...
$('#filter-transaction-type, #filter-account, #filter-payment-method, #filter-tags, #filter-terms, #filter-cleared, #filter-ar-paid, #filter-check-printed, #filter-sales-printed, #filter-sent').on('change', function() {
    if($(this).val() !== 'all') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#run-report-button').on('click', function() {
    ...
    var url = `${base_url}accounting/reports/view-report/${reportId}?`;
    url += filterDate !== 'this-month-to-date' ? `date=${filterDate}&` : '';
    url += filterDate !== 'this-month-to-date' ? `from=${$('#report-period-date-from').val().replaceAll('/', '-')}&to=${$('#report-period-date-to').val().replaceAll('/', '-')}&` : '';
    url += sortBy !== 'default' ? `column=${sortBy}&` : '';
    url += sortIn !== 'asc' ? `order=${sortIn}&` : '';
    ...
    url += $('#allow-filter-customer').prop('checked') && $('#filter-customer').val() !== 'specified' ? `customer=${$('#filter-customer').val()}&` : '';
    ...
    url += $('#allow-filter-tags').prop('checked') && $('#filter-tags').val() !== 'all' ? `tags=${$('#filter-tags').val()}&` : '';
    ...
    url += $('#allow-filter-sales-printed').prop('checked') && $('#filter-sales-printed').val() !== 'all' ? `sales-printed=${$('#filter-sales-printed').val()}&` : '';
    url += $('#allow-filter-sent').prop('checked') && $('#filter-sent').val() !== 'all' ? `sent=${$('#filter-sent').val()}&` : '';
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Transaction List by Customer' ? `report-title=${$('#report-title').val()}&` : '';
...
$('input[name="col_chk"]').on('change', function() {
    ...
    if(groupBy !== 'none') {
        ...
        if($($('#reports-table thead tr td:visible:not([data-name=""])')[0]).index() > $('#reports-table thead tr td[data-name="Amount"]').index()) {
