...
$('#filter-transaction-type, #filter-account, #filter-name, #filter-payment-method').on('change', function() {
    if($(this).val() !== 'all') {
...
$('#filter-num, #filter-ship-via, #filter-po-number, #filter-sales-rep').on('change', function() {
    if($(this).val().trim() !== '') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('input[name="col_chk"]').on('change', function() {
    ...
    var newIndex = $('#reports-table thead tr td[data-name=""]').length > 0 ? 20 : 19;
