...
$('#run-report').on('click', function(e) {
    ...
    var minDaysPastDue = $('#filter-min-days-past-due').val();
...
$('#filter-customer').on('change', function() {
    if($(this).val() !== 'specified') {
        $(`#allow-filter-customer`).prop('checked', true);
    } else {
        $(`#allow-filter-customer`).prop('checked', false);
    }
});
...
$('#filter-ship-via, #filter-po-number, #filter-sales-rep').on('change', function() {
    if($(this).val().trim() !== '') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#run-report-button').on('click', function() {
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Accounts receivable aging detail' ? `report-title=${$('#report-title').val()}&` : '';
