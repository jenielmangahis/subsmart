...
$('select').each(function() {
    ...
    if($(this).attr('id') === 'filter-account' || $(this).attr('id') === 'filter-distribution-account') {
        $(this).select2({
            ajax: {
                url: '/accounting/get-dropdown-choices',
                dataType: 'json',
...
$('#run-report').on('click', function(e) {
    ...
    url += groupBy !== 'account' ? `group-by=${groupBy}&` : '';
...
$('#filter-transaction-type, #filter-distribution-account, #filter-account, #filter-name, #filter-customer, #filter-vendor, #filter-employee, #filter-payment-method, #filter-terms, #filter-product-service, #filter-cleared, #filter-ar-paid, #filter-ap-paid, #filter-check-printed').on('change', function() {
    if($(this).val() !== 'all') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#run-report-button').on('click', function() {
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Transaction Detail by Account' ? `report-title=${$('#report-title').val()}&` : '';
