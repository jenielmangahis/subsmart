...
$('#filter-transaction-type, #filter-distribution-account, #filter-customer, #filter-vendor, #filter-employee, #filter-product-service').on('change', function() {
    if($(this).val() !== 'all') {
...
$('#filter-memo, #filter-num, #filter-amount').on('change', function() {
    if($(this).val().trim() !== '') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#run-report-button').on('click', function() {
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Unbilled charges' ? `report-title=${$('#report-title').val()}&` : '';
...
$('input[name="col_chk"]').on('change', function() {
    ...
    if(groupBy !== 'none') {
        ...
        if($($('#reports-table thead tr td:visible')[0]).data().name === '' && $($('#reports-table thead tr td:visible')[1]).data().name !== 'Amount' && $($('#reports-table thead tr td:visible')[0]).data().name !== 'Balance' ||
