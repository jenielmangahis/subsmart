...
$('#filter-transaction-type, #filter-account, #filter-terms, #filter-cleared, #filter-ap-paid, #filter-check-printed').on('change', function() {
    if($(this).val() !== 'all') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#filter-memo, #filter-num, #filter-amount, #filter-ship-via').on('change', function() {
    if($(this).val().trim() !== '') {
        $(`#allow-${$(this).attr('id')}`).prop('checked', true);
    } else {
        $(`#allow-${$(this).attr('id')}`).prop('checked', false);
...
$('#run-report-button').on('click', function() {
    ...
    url += $('#show-report-title').prop('checked') && $('#report-title').val() !== 'Transaction List by Vendor' ? `report-title=${$('#report-title').val()}&` : '';
