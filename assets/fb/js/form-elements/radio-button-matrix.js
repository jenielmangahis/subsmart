class RadioButtonMatrix extends MatrixElement {
    ...
    getElement() {
        ...
        this.matrix_rows.forEach(row => {
            ...
            this.matrix_columns.forEach(column => {
                element_container.content += `
                    <td>                    
                        <input class="form-check-input position-static mx-auto" type="radio" name="${this.element_type}-${row.id}-${column.id}-${this.id}" id="${this.element_type}-${row.id}-${column.id}-${this.id}">
                    </td>
                `
            });
            element_container.content += `</tr>`;
