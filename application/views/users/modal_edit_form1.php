                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="custom-control custom-switch">
                                ...
                                <label class="custom-control-label allow_clock_in disabled" for="allow_clock_in_input"> <span id="status">Enable</span> user <b>cannot clock in 5 minutes</b> early.</label>
...
<script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.cLocation != 0) {
                        ...
                        if (data.cLocation[0]['allow_gps'] == 0) {
                            $('#allow_clock_in_input').attr('checked', false);
                            $('.allow_clock_in_map').hide();
                            $("#allow_clock_in_input").val(0);
                        } else {
                            $('#allow_clock_in_input').attr('checked', true);
    ...
    function initMap_new_address_map() {
        ...
        map = new google.maps.Map(document.getElementById("add_new_address_map_clock_in"), {
            center: {
        ...
        map = new google.maps.Map(document.getElementById("add_new_address_map_clock_out"), {
            center: {
    ...
    function setMapCenter1(update_the_map, the_lat, the_lng, first = false) {
        ...
        if (update_the_map == "add_new_address_map_clock_out") {
            the_map = add_new_address_map_clock_out;
        } else if (update_the_map == "edit_map") {
            the_map = edit_address_map;
        }
        if (update_the_map == "add_new_address_map_clock_out") {
    ...
    $(document).ready(function() {
        ...
        $("#allow_clock_in_input").on('change', function() {
            ...
            $.ajax({
                url: base_url + 'users/saveClockInOp',
                type: "POST",
                dataType: "json",
                data: {
                    data: $(this).val(),
         ...
         function employee_edit_info_map_setter(user_id=0){
            $.ajax({
                    url: base_url + "users/getData_for_clock_in_out_location_std_return",
                    type: "POST",
                    dataType: "json",
                    data: {
                        user_id: user_id
                    ...
                    success: function(data) {
                        if (data.cLocation > 0) {
                            current_lat_clock_in= data.clock_in_latitude;
                            current_lng_clock_in = data.clock_in_longtitude;
                            current_lat_clock_out = data.clock_out_latitude;
                            current_lng_clock_out = data.clock_out_longtitude;
                            setMapCenter("add_new_address_map_clock_in", data.clock_in_latitude, data.clock_in_longtitude, first = false);
                            setMapCenter1("add_new_address_map_clock_out", data.clock_out_latitude, data.clock_out_longtitude, first = false);
                        }
                    }
            });
        }
