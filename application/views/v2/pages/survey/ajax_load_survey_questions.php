...
<?php foreach($questions as $key =>  $question){ 
	if($question->template_id == 20){
		...
		<div id="container-<?= $question->id ?>" class="col-sm-12">
			<div class="card mb-2" style="background-color: #32243d; color:#ffffff;">
					...
					<?= form_open("survey/update/question/".$question->id."", array('id'=>'frm-update-question')); ?>
						...
						<div class="d-flex justify-content-between">
							...
							<div class="btn-group justify-content-right">
								<div>
									<a class="nsm-button primary btn-sm text-white btn-question-action" href="<?php echo base_url()?>survey/<?= $survey->id?>" id="btn-question-delete"  data-id="<?= $question->id ?>"><i class='bx bx-trash' style="font-size: 19px;position: relative;top: 1px;"></i></a>
									<a class="nsm-button primary btn-sm text-white btn-question-action" type="button" data-bs-toggle="collapse" data-bs-target="#navbarToggleExternalContent<?= $question->id ?>" aria-controls="navbarToggleExternalContent<?= $question->id ?>" aria-expanded="false" aria-label="Toggle navigation"><i class='bx bx-info-circle' style="font-size: 19px;position: relative;top: 1px;"></i></a>
								</div>
							</div>
						</div>
						...
						<div class="question-input-box" >
										...
										foreach($question->questions as $option){
											?>
												<div class="d-flex w-100 justify-content-between choice-container q-choice-container-<?= $option->id; ?>" style="margin:10px 0px; height:44px;">
													...
													<button id="btn-delete-option" data-id="<?= $option->id?>" class="nsm-button primary btn-delete-choice" type="button" name="button"><i class="bx bx-trash"></i></button>
												</div>
											<?php
										}
								}else{ ?>
							...
							<div class="collapse question-toggle" id="navbarToggleExternalContent<?= $question->id ?>">
								<ul class="list-group">
									<!-- image upload field -->
