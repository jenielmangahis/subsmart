...
<style>
    ...
    .chatbot_chat_content::-webkit-scrollbar,
    .client_chatContent::-webkit-scrollbar {
        width: 10px; 
    }

    .chatbot_chat_content::-webkit-scrollbar-track,
    .client_chatContent::-webkit-scrollbar-track {
        background: transparent;
    }

    .chatbot_chat_content::-webkit-scrollbar-thumb,
    .client_chatContent::-webkit-scrollbar-thumb {
        background: rgba(0, 0, 0, 0.3); 
    }

    .chatbot_chat_content::-webkit-scrollbar-thumb:hover,
    .client_chatContent::-webkit-scrollbar-thumb:hover {
        ...
        <div class="container">
            ...
            <div class="row client_realtimeChat" style="display: none;">
                ...
                <div class="col-12-md client_chatContainer">
                    <div id="chatbox" class="card">
                        <div class="card-header client_chatboxHeader d-flex align-items-center msgPadding text-white border-bottom-0">
                            <img class="client_realtimeImage" src="https://cdn-icons-png.flaticon.com/512/10306/10306707.png">
            ...
            <div class="row chatBotMessenger" style="display: none;">
                ...
                <div class="col-12-md chatbox_container">
                    <div id="chatbox" class="card">
                        <div class="card-header chatbox_header d-flex align-items-center msgPadding text-white border-bottom-0">
...
<script>
    ...
    var clientChatBoxPusher = null;
    var clientChatBoxChannel = null;
                ...
                if (data.length > 0) {
                    ...
                    $('.client_chatContent').append(`
                        <div class="position-relative fetchingConversationLoader">
                            <div class="receive_chat d-flex flex-row justify-content-center">
                                <div class="msgPadding me-3">
                                    <i class="mb-0 text-muted">Start a conversation with Tech Support.</i>
                                </div>
                            </div>
                        </div>
                    `);
    ...
    function send_message(formElement) {
        const message = $('input[name="client_chatMessage"]').val();
        const bgcolor = "#6a4a86";
        ...
        $.ajax({
            ...
            success: function() {
                ...
                $('input[name="client_chatMessage"]').val(null);
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.openChatbotButton', function() {
            ...
            let chatContent_height = $(window).height() - 298;
            $('.chatbot_chat_content').css('height', chatContent_height + 'px');
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Entry Saved!",
                        html: "Call support has been scheduled.",
                        showConfirmButton: true,
                        confirmButtonText: "Okay",
                    });
                },
        ...
        $(document).on('submit', '#chatbot_chat_form', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.chatbot_chat_content').append(`
                        <div class="send_container position-relative">
                            <div class="send_chat d-flex flex-row justify-content-end">
                                <div class="msgPadding send_chat_container" style="background-color: ${bgcolor}21;">
                                    <span class="mb-0 send_chat_message">${chatMessage}</span>
                                </div>
                            </div>
                        </div>
                    `).scrollTop($('.chatbot_chat_content')[0].scrollHeight);
        ...
        $(document).on('click', '.openRealTimeChat', function() {
            ...
            if (clientChatBoxPusher) {
                clientChatBoxPusher.disconnect();
            }

            Pusher.logToConsole = false;
            clientChatBoxPusher = new Pusher('33cfc0c407521da10fe6', { cluster: 'ap1' });
            clientChatBoxChannel = clientChatBoxPusher.subscribe('realtime_chat_channel');

            clientChatBoxChannel.unbind(`chat_${<?php echo $user_id; ?>}_event`);
            clientChatBoxChannel.bind(`chat_${<?php echo $user_id; ?>}_event`, function(data) {
            ...
            const chatContent_height = $(window).height() - 340;
            $('.client_chatContent').css('height', `${chatContent_height}px`);
        });

        $('input[name="client_chatMessage"]').on('focus blur', function(event) {
            $.post(BASE_URL + '/TechSupportSidebar/chatRequestProcess/client_typing', {
