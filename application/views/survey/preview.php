<html>
  ...
  <body>
    ...
    <div class="d-flex h-100 justify-content-center align-items-center">
      <?php
        if($survey_theme !== null){
          ?>
            <div>
                <!-- <img class="theme-image image-set-background" src="<?=base_url()?>uploads/survey/image_db/coffee.jpg" alt="coffee" style="width: 100%"> -->
            </div>
                    ...
                    <div id="survey-wrapper">
                      ...
                      <?php foreach($questions as $key => $question){ ?>                      
                        <div id="question-<?= $key ?>" class="col-sm-3 <?= ($key != 0) ? "d-none" : "" ?>">
                          ...
                          <?php endif; ?>
                          <button id="btn-next" data-id="<?= $key ?>"  class="btn btn-md btn-primary mt-3" style="background-color: <?= $survey_theme !== null ? $survey_theme->sth_primary_color : ""?>; color: <?= $survey_theme !== null ? $survey_theme->sth_text_color : ""?>"><?= ($question->custom_button_text == "" || $question->custom_button_text == null) ? "Next" : $question->custom_button_text ?></button>
                          <?php if($key >= 1){
                            ?>
                      ...
                      <div id="question-<?= count($questions) ?>" class="col-sm-6 d-none">
                        ...
                        <?php
                          if(strpos(uri_string(), 'preview') !== false){
                            ?>
                              <p id="preview-end-note" class="border text-center rounded" >This is the end of the survey. <a id="from-top" style="color: <?= $survey_theme->sth_text_color?>" href="#">Back to top</a> or <a style="color: <?= $survey_theme->sth_text_color?>" href="<?= ($survey->canRedirectOnComplete == true && $survey->redirectionLink != "") ? ($survey->redirectionLink) : (base_url()."survey/edit/". $survey->id) ?>">redirect to another page.</a></p>
                            <?php
                          }else{
                            ?>
      ...
      <div class="page-counter unselectable">
        <?php
          if($survey->hasProgressBar == true){
            ?>
              <div class="progress">
                <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100" style="width: 75%; background-color: <?= $survey_theme->sth_secondary_color?>"></div>
              </div>
            <?php
          }
        ?>
    ...
    <script>
      let pageNumber = 1;
      let numQuestions = <?=count($questions)?>;
