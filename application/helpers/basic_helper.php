...
function get_invoice_amount($type)
{
    ...
    switch ($type) {
        ...
        case "unpaid":
            ...
            return number_format($total_invoice, 2, '.', ',');
        case "unpaid_thumb_widget":
            $resultInvoice  = $CI->invoice_model->getCompanyTotalAmountUnPaidInvoicesV2($company_id);
            $total_invoice = 0;
            if( $resultInvoice ){
                $total_invoice = $resultInvoice->total_amount;
            }
...
if (!function_exists('is_admin_logged')) {
    ...
    function exempted_company_ids(){
        $exempted_company_ids = [1,24,31,58,52,85,3644,3696,139];
                ...
                if( $user_id > 0 ){
                    ...
                    if( $itemCreator ){
                        if( $itemCreator->mobile != '' ){
                            ...
                            $filter[] = ['field' => 'mobile_number', 'value' => $mobile_number];
