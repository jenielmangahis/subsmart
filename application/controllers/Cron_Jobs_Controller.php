...
class Cron_Jobs_Controller extends CI_Controller
{
    ...
    /* 
    Must trigger every first day of the month
    */
    public function cronCustomerSubscription()
    {
    ...
    //Note: backup dated Jan 31, 2025
	public function cronCustomerSubscriptionV2Backup(){		
		$this->load->model('AcsCustomerSubscriptionBilling_model');
        $this->load->model('AcsProfile_model');
        $this->load->model('Invoice_model');
        $this->load->model('Invoice_settings_model');
	...
	public function cronCustomerSubscriptionV2Backup03202025(){		
		$this->load->model('AcsCustomerSubscriptionBilling_model');
        $this->load->model('AcsProfile_model');
        $this->load->model('Invoice_model');
        $this->load->model('Invoice_settings_model');
    ...
    public function cronCustomerSubscriptionV2(){
		$this->load->model('AcsCustomerSubscriptionBilling_model');
        $this->load->model('AcsProfile_model');
        $this->load->model('Invoice_model');
        $this->load->model('Invoice_settings_model');
    ...
    public function cronUpdateInvoice() {
        ...
        /**
         * Update recurring invoice late computation - end
         */
    }    

    /**
     * Note:
     * -Done: late fee adjustment
     * -Todo: implement stripe payment if pay method is CC
     */
	public function cronCustomerSubscriptionV3(){		
		$this->load->model('AcsCustomerSubscriptionBilling_model');
        $this->load->model('AcsProfile_model');
        $this->load->model('Invoice_model');
        $this->load->model('Invoice_settings_model');
		...
		foreach( $activeSubscriptions as $as ) {
            ...
            if( $as->mmr > 0 && $customer ){
                ...
                //Check for unpaid subscription with customer billing method is credit card
                $is_stripe_payment_successfull = false;
                if($as->bill_method != null && $as->bill_method == 'CC') {
                    /**
                     * Todo: process stripe payment here
                     */
                    if($is_stripe_payment_successfull) {

                        $unpaidSubscriptionsDetails = $this->AcsCustomerSubscriptionBilling_model->getUnpaidDetailsByCustomerId($as->fk_prof_id);

                        $invdata = [
                            'id' => $unpaidSubscriptionsDetails->id,
                            'status' => 'Paid'
                        ];           
                        $invoice_paid = $this->customer_ad_model->update_data($invdata, 'invoices', 'id');	                          
                        if($invoice_paid) {
                            $payment_data = [
                                'user_id' => '',
                                'company_id' => $unpaidSubscriptionsDetails->company_id,
                                'customer_id' => $unpaidSubscriptionsDetails->customer_id,
                                'invoice_id' => $unpaidSubscriptionsDetails->id,
                                'invoice_amount' => $unpaidSubscriptionsDetails->grand_total,
                                'invoice_tip' => 0,
	...
	public function cronCustomerSubscriptionV2test(){		
		$this->load->model('AcsCustomerSubscriptionBilling_model');
        $this->load->model('AcsProfile_model');
        $this->load->model('Invoice_model');
        $this->load->model('Invoice_settings_model');
        ...
        $activeSubscriptions = $this->customer_ad_model->getAllActiveSubscriptionsWithSub14DaysByCompanyId($current_date, $company_id);	 
		foreach( $activeSubscriptions as $as ) {
            $customer = $this->AcsProfile_model->getByProfId($as->fk_prof_id);    
