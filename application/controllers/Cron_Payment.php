        ...
        foreach( $data as $d ){
            ...
            if( $total_amount > 0 ){
                ...
                if( $createSale['success'] == 1 ){
                    //Update billing
                    $transaction_data = array();
                    $transaction_data['next_subscription_billing_date'] = date("n/j/Y",strtotime("+" . $d->billing_frequency . " months"));
                    $this->general->update_with_key_field($transaction_data, $d->bill_id, 'acs_billing', 'bill_id');
                    ...
                    if( $d->billing_frequency == 1 ){
                        $num_frequency = 'One Time Only';
                    }elseif( $d->billing_frequency == '12' ){
                        $num_frequency = 'Every 1 Year';
                    }else{
                        $num_frequency = 'Every '.$d->billing_frequency.' Month(s)';
                    }
                    ...
                    $subscription_details['num_frequency'] = $num_frequency;
                    $subscription_details['notes'] = 'Payment for ' .$d->transaction_category. ' for the month of ' . date("M/Y");
