...
class Debug extends MY_Controller {
    ...
    public function alarmApi()
    {
        ...
        // $customers = $alarmApi->getCustomers($token['token'], 0, 0);
        // echo "<pre>";
        // print_r($customers);
        // foreach($customers['customers'] as $c){
        //     $data = $alarmApi->getCustomerServicePlans($c->customerId, $token['token']);
        // }     
        ...
        $data = $alarmApi->getCustomerServicePlans($customer_id, $token['token']);
        echo "<pre>";
        print_r($data);
    ...
    public function fixActivityLogs()
    {
        ...
        foreach($logs as $log){
            ...
            if( $company_id > 0 ){
                ...
                $this->Activity_model->delete($log->id);
                $total_deleted++;
            }         
        }
    ...
    public function fixAdiCustomerAccountCostV1()
    {
        $this->load->model('AcsProfile_model');
    ...
    public function fixAdiCustomerServiceProvider()
    {
        $this->load->model('AcsProfile_model');
        $this->load->model('AcsAlarm_model');
        ...
        foreach($customers as $c){
            ...
            if($alarm){                
                //if( $c->prof_id == 31100 ){
                    ...
                    //     if( $systemPackage ){
                    //         if( $service_provider == 'Alarm.com' ){
                    //             $pass_thru_cost = $systemPackage->alarmcom_cost;
                    //         }elseif( $service_provider == 'AlarmNet' ){
    ...
    public function fixAdiCustomerAccountCostV2()
    {
        $this->load->model('AcsProfile_model');
        $this->load->model('AcsAlarm_model');
        $this->load->model('SystemPackageType_model');
