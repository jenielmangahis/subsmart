...
class Chart_of_accounts extends MY_Controller {
    ...
    private function bill_registers($accountId, $data = [])
    {
        ...
        $billItems = $this->chart_of_accounts_model->get_vendor_transaction_category_registers($accountId, 'Bill');
    ...
    private function vendor_credit_registers($accountId, $data = [])
    {
        ...
        $vendorCredits = $this->chart_of_accounts_model->get_vendor_transaction_category_registers($accountId, 'Vendor Credit');
    ...
    private function save_cc_credit($accountId, $ccCreditId, $data)
    {
        ...
        $ccCreditData = [
            'payment_date' => date('Y-m-d', strtotime($data['date'])),
            'ref_no' => $accountId === $ccCredit->bank_credit_account_id && !isset($data['child_id']) ? $data['ref_no'] : $ccCredit->ref_no,
            'payee_type' => $payee[0],
            'payee_id' => $payee[1],
            // 'attachments' => $data['attachments'] !== null ? json_encode($data['attachments']) : null,
