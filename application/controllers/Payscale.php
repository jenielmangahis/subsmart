...
class Payscale extends MY_Controller
{
    ...
    public function calculatePayroll()
    {
        ...
        $payscale_employees = $this->db->get()->result();
    
        $this->db->distinct();
        $this->db->select("company_id, employee_id");
        $this->db->from('employee_commission_summary');
        $this->db->where("DATE_FORMAT(date_created, '%Y-%m') = ", $currentMonth);
        $commission_employees = $this->db->get()->result();
    
        $employees = [];
        $index = [];
        foreach (array_merge($payscale_employees, $commission_employees) as $emp) {
            $key = $emp->company_id . '-' . $emp->employee_id;
            if (!isset($index[$key])) {
                $employees[] = $emp;
                $index[$key] = true;
            }
        ...
        foreach ($employees as $emp) {
            ...
            if ($pending_payroll) {
                ...
                $this->db->where('company_id', $company_id);
                $this->db->where('employee_id', $employee_id);
                $this->db->where('payroll_id IS NULL');
            ...
            if ($payroll_id) {
                ...
                $final_total_amount = $payscale_total + $commission_total;
