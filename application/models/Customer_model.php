...
class Customer_model extends MY_Model
{
    ...
    public function getAllCustomerByCustomerType($search = null, $company_id, $customer_type, $filter_status = null) {
        $this->db->select('*, acs_billing.mmr AS customer_mmr, concat(acs_profile.mail_add, " ", acs_profile.city, " ", acs_profile.state," , ",acs_profile.zip_code) AS customer_address, acs_office.fk_sales_rep_office AS fk_sales_rep_office');
        $this->db->from($this->table);
    ...
    public function getAllCommercialCustomers($search = null, $company_id, $customer_type, $filter_status = null) {
        ...
        $this->db->where('acs_profile.customer_type', $customer_type);
        $this->db->join('acs_billing', 'acs_billing.fk_prof_id = acs_profile.prof_id', 'left');
    ...
    public function getAllCommercialCustomersIsNotArchived($search = null, $company_id, $customer_type, $filter_status = null) {
        $this->db->select('*, acs_billing.mmr AS customer_mmr, concat(acs_profile.mail_add, " ", acs_profile.city, " ", acs_profile.state," , ",acs_profile.zip_code) AS customer_address');
        $this->db->from($this->table);
        $this->db->where('acs_profile.company_id', $company_id);
        $this->db->where('acs_profile.customer_type', $customer_type);
        ...
        $this->db->where("(acs_profile.first_name != '')");
        $this->db->where("(acs_profile.last_name != '')");
        $this->db->where("acs_profile.is_archvied", 0);
...
//        echo $this->db->last_query(); die;
    ...
    {
        $this->db->where('prof_id', $id);      
        $this->db->update($this->table, array("is_favorite" => 1, 'updated_at' => NULL));        
    }
    ...
    public function getAllFavoritesByCompanyId($company_id, $filter = array())
    {
