...
class FB_template_elements_model extends MY_Model {

	public $table = 'form_template_elements';
	...
	public function __construct()
	{
		...
		// $this->load->model('FB_template_element_rules_model', 'form_template_element_rules');
	}
	...
	function getFormElements($id) {
		try {
			...
			$res = [
				...
				'code'	=> 200,
				'message'	=> 'Templates element fetched successfuly'
			];
		} catch(\Exception $e) {
			$res = [
				'data' 	=> [],
				'code'	=> 500,
				'message'	=> 'Error fetching form template elements. please try again later or contact customer support.'
			];
		}
		...
		try {
			...
			if($data['choices']) {
				foreach ($data['choices'] as $i => $choice) {
					...
					$this->db->insert($this->form_template_element_choices->table, $choiceData);			
				}
			}
			...
			if($data['choices_and_prices']) {
				foreach ($data['choices_and_prices'] as $i => $row) {
					...
					$this->db->insert($this->form_template_element_items->table, $items_data);			
				}
			}

			if($data['matrix_row']) {
				foreach ($data['matrix_row'] as $i => $matrixRow) {
					$matrixRowData = [
						'id'			=> null,
						'element_id' 	=> $newID,
						'text'	=> ( gettype($matrixRow) === 'array' ? $matrixRow['text'] : $matrixRow )
					];
					$this->db->insert($this->form_template_element_matrix_rows->table, $matrixRowData);			
				}
				foreach ($data['matrix_column'] as $i => $matrixColumn) {
					$matrixColumnData = [
						'id' 	=> null,
						'element_id' 	=> $newID,
						'text'	=> ( gettype($matrixColumn) === 'array' ? $matrixColumn['text'] : $matrixColumn )
					];
					$this->db->insert($this->form_template_element_matrix_columns->table, $matrixColumnData);			
				}	
			}		
