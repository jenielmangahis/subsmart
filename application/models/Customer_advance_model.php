...
class Customer_advance_model extends MY_Model
{
    ...
    public function update_data($input, $tablename, $update_id)
    {
        ...
        if ($this->db->update($tablename, $input, [$update_id => $id])) {
            return true;
        } else {
            return false;
        }
    ...
    public function get_leads_data_graph()
    {
        $cid = logged('company_id');
        $this->db->select('COUNT(ac_leadtypes.lead_name) AS total_leads, ac_leadtypes.lead_name'); // Count lead names and group by lead name
        $this->db->from('ac_leads');
        $this->db->join('users', 'users.id = ac_leads.fk_sr_id', 'left');
        $this->db->join('ac_leadtypes', 'ac_leadtypes.lead_id = ac_leads.fk_lead_type_id', 'left');
        $this->db->where('ac_leads.company_id', $cid);
        $this->db->group_by('ac_leadtypes.lead_name');
        $query = $this->db->get();
    ...
    public function get_all_leads_data($filters = [], $search = [])
    {
        $this->db->from('ac_leads');
    ...
    public function getCustomerLists($param = null, $start = 0, $length = 0, $record = 0 ,  $filter_status = null, $customer_type = '')
    {
        ...
        if ($cid == 58) {
            ...
            acs_alarm.system_type,acs_alarm.comm_type,acs_office.entered_by,acs_office.lead_source,acs_profile.city,acs_profile.state,users.LName,users.FName,acs_profile.customer_type,
            ...
            acs_office.fk_sales_rep_office,acs_info_solar.proposed_solar,acs_info_solar.proposed_payment,acs_profile.company_id, acs_profile.customer_no');
            $this->db->join('acs_info_solar', 'acs_info_solar.fk_prof_id = acs_profile.prof_id', 'left');
        } else {
            $this->db->select('users.id,acs_profile.prof_id,acs_profile.is_favorite,acs_profile.first_name,acs_profile.last_name,acs_profile.email,acs_profile.phone_m,acs_profile.status,acs_b.mmr,
            acs_alarm.system_type,acs_alarm.comm_type,acs_office.entered_by, CONCAT(users.FName, " ", users.LName) AS entered_by2, acs_office.lead_source,acs_profile.city,acs_profile.state,users.LName,users.FName,acs_profile.customer_type,
            acs_profile.business_name,acs_office.technician,acs_b.transaction_amount as total_amount,industry_type.name AS industry_type, acs_profile.industry_type_id,acs_office.fk_sales_rep_office,acs_profile.company_id, acs_profile.customer_no');
        }
    ...
    public function check_if_user_exist($params = [], $tablename)
    {
        $this->db->select('*');
        $this->db->from($tablename);
    ...
    public function get_all($limit = false, $start = 0, $sort = 'ASC', $tablename, $orderBy, $search = [])
    {
        if (!empty($search)) {
    ...
    public function getJobSalesTechPaid($customer_id)
    {
        ...
        if ($userResult_1->payscale == 'Base (Hourly Rate)') {
            $totalSalesRepPaid += $userResult_1->base_hourly;
        }
        if ($userResult_1->payscale == 'Base (Weekly Rate)') {
            $totalSalesRepPaid += $userResult_1->base_weekly;
        }
        if ($userResult_1->payscale == 'Base (Montly Rate)') {
            $totalSalesRepPaid += $userResult_1->base_monthly;
        }
        // =================
        $this->db->select('payscale.payscale_name AS payscale, users.base_hourly AS base_hourly, users.base_weekly AS base_weekly, users.base_monthly AS base_monthly');
        $this->db->from('users');
        ...
        if ($userResult_2->payscale == 'Base (Hourly Rate)') {
            $totalTechRepPaid += $userResult_2->base_hourly;
        }
        if ($userResult_2->payscale == 'Base (Weekly Rate)') {
            $totalTechRepPaid += $userResult_2->base_weekly;
        }
        if ($userResult_2->payscale == 'Base (Montly Rate)') {
            $totalTechRepPaid += $userResult_2->base_monthly;
        }
        // =================
        $this->db->select('payscale.payscale_name AS payscale, users.base_hourly AS base_hourly, users.base_weekly AS base_weekly, users.base_monthly AS base_monthly');
        $this->db->from('users');
        ...
        if ($userResult_3->payscale == 'Base (Hourly Rate)') {
            $totalTechRepPaid += $userResult_3->base_hourly;
        }
        if ($userResult_3->payscale == 'Base (Weekly Rate)') {
            $totalTechRepPaid += $userResult_3->base_weekly;
        }
        if ($userResult_3->payscale == 'Base (Montly Rate)') {
            $totalTechRepPaid += $userResult_3->base_monthly;
        }
        // =================
        $this->db->select('payscale.payscale_name AS payscale, users.base_hourly AS base_hourly, users.base_weekly AS base_weekly, users.base_monthly AS base_monthly');
        $this->db->from('users');
        ...
        if ($userResult_4->payscale == 'Base (Hourly Rate)') {
            $totalTechRepPaid += $userResult_4->base_hourly;
        }
        if ($userResult_4->payscale == 'Base (Weekly Rate)') {
            $totalTechRepPaid += $userResult_4->base_weekly;
        }
        if ($userResult_4->payscale == 'Base (Montly Rate)') {
            $totalTechRepPaid += $userResult_4->base_monthly;
        }
        // =================
        $this->db->select('payscale.payscale_name AS payscale, users.base_hourly AS base_hourly, users.base_weekly AS base_weekly, users.base_monthly AS base_monthly');
        $this->db->from('users');
        ...
        if ($userResult_5->payscale == 'Base (Hourly Rate)') {
            $totalTechRepPaid += $userResult_5->base_hourly;
        }
        if ($userResult_5->payscale == 'Base (Weekly Rate)') {
            $totalTechRepPaid += $userResult_5->base_weekly;
        }
        if ($userResult_5->payscale == 'Base (Montly Rate)') {
            $totalTechRepPaid += $userResult_5->base_monthly;
        }
        // =================
        $this->db->select('payscale.payscale_name AS payscale, users.base_hourly AS base_hourly, users.base_weekly AS base_weekly, users.base_monthly AS base_monthly');
        $this->db->from('users');
        ...
        if ($userResult_6->payscale == 'Base (Hourly Rate)') {
            $totalTechRepPaid += $userResult_6->base_hourly;
        }
        if ($userResult_6->payscale == 'Base (Weekly Rate)') {
            $totalTechRepPaid += $userResult_6->base_weekly;
        }
        if ($userResult_6->payscale == 'Base (Montly Rate)') {
            $totalTechRepPaid += $userResult_6->base_monthly;
        }
        // =================
    ...
    public function getAllLeadsByCompanyId($company_id, $filters = [])
    {
        $this->db->select('*');
        $this->db->from('ac_leads');
        $this->db->where('company_id', $company_id);
    ...
    public function getDuplicateListData($company_id, $customerData, $searchMode, $searchType, $businessNameSearch, $firstNameSearch, $lastNameSearch)
    {
        if ($customerData == 'customer_with_count') {
            if ($searchType == 'Residential') {
                ...
                $query = $this->db->get();
            } elseif ($searchType == 'Commercial' || $searchType == 'Business') {
                $this->db->select(' acs_profile.prof_id, acs_profile.company_id, acs_profile.customer_type, TRIM(acs_profile.business_name) AS business_name, TRIM(acs_profile.first_name) AS first_name, TRIM(acs_profile.last_name) AS last_name, COUNT(*) AS total ');
                $this->db->from('acs_profile');
                $this->db->where('acs_profile.company_id', $company_id);
        ...
        } elseif ($customerData == 'all_duplicated_customer') {
        ...
        foreach ($tableList as $tableLists) {
            foreach ($duplicateID as $duplicateIDs) {
                if ($tableLists->TABLE_NAME != 'acs_papers') {
                    ...
                    $status &= $this->db->update($tableLists->TABLE_NAME, ['customer_id' => $entryID]);
                }
            }
        }
    ...
    public function getAllActiveSubscriptionsWithSub14Days($current_date) {
        ...
        //$this->db->where_in('acs_billing.fk_prof_id', [31045]); //for testing only  
    ...
    public function getAllActiveSubscriptionsWithSub14DaysByCompanyId($current_date, $company_id) {
    ...
    public function getActiveSubscriptionsByCustomerId($customer_id)
    {
        ...
        $this->db->from('acs_billing');
        $this->db->join('acs_profile', 'acs_billing.fk_prof_id = acs_profile.prof_id', 'left');
        $this->db->where('acs_billing.fk_prof_id =', $customer_id);  
        $query = $this->db->get();
