...
class Customer_advance_model extends MY_Model {
    ...
    public function getCustomerLists($param = null, $start = 0, $length = 0, $record = 0)
    {
        ...
        if($record != 0){
            ...
            if( $param['search'] != '' ){
                $this->db->group_start();
                    $this->db->or_like('acs_profile.last_name', $param['search'], 'LEFT');    
                    $this->db->or_like('acs_profile.first_name', $param['search'], 'LEFT');    
                    //$this->db->or_like('acs_profile.email', $param['search'], 'both'); 
                    $this->db->or_like('acs_profile.business_name', $param['search'], 'LEFT'); 
                $this->db->group_end();   
            }  
            $query = $this->db->get();
