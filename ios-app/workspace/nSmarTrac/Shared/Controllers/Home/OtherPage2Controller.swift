//
//  OtherPage2Controller.swift
//  nSmarTrac
//
//  Created by Jonah Abanil on 14/06/2020.
...
class OtherPage2Controller: UIViewController {
    ...
    func initMenu() {
        if let mData = UserDefaults.standard.value(forKey: "HOME_SCREEN3_ICON") as? [[String : Any]],
           let mPreset = UserDefaults.standard.value(forKey: "PRESET_HOME_SCREEN3_ICON") as? [[String : Any]] {
            ...
            dictionary.append(MenuItem.trac360)
            dictionary.append(MenuItem.tasks)
            dictionary.append(MenuItem.timeClock)
            dictionary.append(MenuItem.routePlanner)
            dictionary.append(MenuItem.bulletin)
            dictionary.append(MenuItem.collageMaker)
            dictionary.append(MenuItem.costEstimator)
            dictionary.append(MenuItem.virtualEstimator)
            
            objects = Menu.models(from: dictionary)
            presets = Menu.models(from: dictionary)
            processMenu(objects)
            
            UserDefaults.standard.set(objects.toJSON(), forKey: "HOME_SCREEN3_ICON")
            UserDefaults.standard.set(objects.toJSON(), forKey: "PRESET_HOME_SCREEN3_ICON")
...
extension OtherPage2Controller: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            ...
            parent.setViewControllers([HomeTabController().orderedViewControllers[1]], direction: .reverse, animated: true, completion: nil)
        } else {
            let item = myMenu[indexPath.row]
    ...
    func collectionView(_ collectionView: UICollectionView, moveItemAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        UserDefaults.standard.set(myMenu.toJSON(), forKey: "HOME_SCREEN3_ICON")
        UserDefaults.standard.synchronize()
    }
}
