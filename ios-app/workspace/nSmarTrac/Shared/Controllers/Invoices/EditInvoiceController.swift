...
class EditInvoiceController: UITableViewController {
    ...
    func loadData() {
        ...
        invoiceTypeField.text           = item.invoice_type
        workOrderNumField.text          = item.work_order_number
        invoiceNumField.text            = item.invoice_number
        poNumField.text                 = item.po_number
        dateIssuedField.date            = Date(fromString: item.date_issued, format: DateHelper.dateFormatType)!
        dueDateField.date               = Date(fromString: item.due_date, format: DateHelper.dateFormatType)!
        depositRequestField.text        = item.deposit_request
        messageToCustomerField.text     = item.message_to_customer
        termsField.text                 = item.terms_and_conditions
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
        ...
        App.shared.api.putInvoice(item.id.intValue, params: invoice, flag: "DELETE_ITEMS") { (success, error) in
            ...
            for (key, value) in grouped {
                // create json
                let item: Parameters = ["invoice_id": self.item.id, "items_id": key, "qty": value.count]
                // enter group
                group.enter()
            ...
            for image in self.selectedImages {
                ...
                App.shared.api.uploadInvoicePhoto(image, imageName: "\(self.item!.invoice_number)_photo_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                    SVProgressHUD.setDefaultMaskType(.none)
                    SVProgressHUD.dismiss()
                    guard error == nil else {
                        return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
...
extension EditInvoiceController: TLPhotosPickerViewControllerDelegate {
...
extension EditInvoiceController: FSPagerViewDataSource, FSPagerViewDelegate {
