...
class InvoicesController: UIViewController {
    ...
    func initNavBar() {
        ...
        // set title
        self.navigationItem.title = "My Invoices"
    }
    ...
    func initScrollableSegmentControl() {
        ...
        segmentedControl.insertSegment(withTitle: "ALL", at: 0)
        segmentedControl.insertSegment(withTitle: "DUE", at: 1)
        segmentedControl.insertSegment(withTitle: "PAID", at: 2)
...
extension InvoicesController: FloatyDelegate {
    ...
    func initFAB() {
        ...
        floaty.addItem("Filter", icon: UIImage.fontAwesomeIcon(name: .filter, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_InvoiceFilterController")
            self.floaty.close()
        })
        floaty.addItem("Add", icon: UIImage.fontAwesomeIcon(name: .edit, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_AddInvoiceController")
            self.floaty.close()
        })
