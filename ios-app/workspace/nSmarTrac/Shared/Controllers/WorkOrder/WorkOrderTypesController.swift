//
//  WorkOrderTypesController.swift
//  nSmarTrac
//
...
class WorkOrderTypesController: UIViewController {
...
extension WorkOrderTypesController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // check
        if indexPath.section == groupedItems.count {
            // Configure the cell...
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let edit = UIContextualAction(style: .normal, title: "") {  (contextualAction, view, boolValue) in
            ...
            let viewController = self.storyboard?.instantiateViewController(withIdentifier: "sb_EditWorkOrderTypeController") as? EditWorkOrderTypeController
        ...
        if App.shared.user!.role.intValue <= 3 {
            ...
            let delete = UIContextualAction(style: .destructive, title: "") {  (contextualAction, view, boolValue) in
                ...
                let yesAction = UIAlertAction(title: "Yes", style: .default) { (alertAction) -> Void in
                    ...
                    App.shared.api.deleteWorkOrderType(item.id.intValue) { (success, error) in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // check
        if indexPath.section == groupedItems.count {
            return self.pushTo(storyBoard: "Main", identifier: "sb_AddWorkOrderController")
        }
        ...
        if let template = item.template {
            ...
            Utils.shared.showAlertWithMessage(title: "Sorry!", message: "There is no template saved on this work order type...")
        }
    }
}
...
extension WorkOrderTypesController: FloatyDelegate {
    ...
    func initFAB() {
        ...
        floaty.addItem("Add", icon: UIImage.fontAwesomeIcon(name: .edit, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_AddWorkOrderTypeController")
            self.floaty.close()
        })
