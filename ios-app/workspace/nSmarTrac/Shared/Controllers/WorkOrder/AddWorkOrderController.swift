...
class AddWorkOrderController: UITableViewController {
    ...
    var template: WorkOrderTemplate?
    ...
    func loadData() {
        // handle nil
        if let item = template {
            print(template)
            headerField.text                = item.header
            ...
            termsAndConditionsField.text    = item.terms_and_conditions
            termsOfUserField.text           = item.terms_of_use
                ...
                App.shared.api.getWorkOrder(id.intValue, completion: { (item, error) in
                    ...
                    if let item = item {
                        ...
                        if let image = self.companySignatureView.getCroppedSignature() {
                            ...
                            App.shared.api.uploadWorkOrderSignature(image, imageName: "\(item.work_order_number)_company_rep_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                                SVProgressHUD.setDefaultMaskType(.none)
                                SVProgressHUD.dismiss()
                                guard error == nil else {
                                    return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
                        ...
                        if let image2 = self.primarySignatureView.getCroppedSignature() {
                            ...
                            SVProgressHUD.setDefaultMaskType(.clear)
                            SVProgressHUD.show(withStatus: "Uploading...")
                            App.shared.api.uploadWorkOrderSignature(image2, imageName: "\(item.work_order_number)_primary_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                                SVProgressHUD.setDefaultMaskType(.none)
                                SVProgressHUD.dismiss()
                                guard error == nil else {
                                    return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
                        ...
                        if let image3 = self.secondarySignatureView.getCroppedSignature() {
                            ...
                            SVProgressHUD.setDefaultMaskType(.clear)
                            SVProgressHUD.show(withStatus: "Uploading...")
                            App.shared.api.uploadWorkOrderSignature(image3, imageName: "\(item.work_order_number)_secondary_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                                SVProgressHUD.setDefaultMaskType(.none)
                                SVProgressHUD.dismiss()
                                guard error == nil else {
                                    return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
                        ...
                        for image in self.selectedImages {
                            ...
                            SVProgressHUD.setDefaultMaskType(.clear)
                            SVProgressHUD.show(withStatus: "Uploading...")
                            App.shared.api.uploadWorkOrderPhoto(image, imageName: "\(item.work_order_number)_photo_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                                SVProgressHUD.setDefaultMaskType(.none)
                                SVProgressHUD.dismiss()
                                guard error == nil else {
                                    return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
    ...
    @IBAction func saveAsTemplateButtonTapped(_ sender: Any) {
        // get event color
        let eventColor = EventColor.colors[defaultRadio.selected()?.tag ?? 0]
        ...
        // go to select screen
        let viewController = self.storyboard?.instantiateViewController(withIdentifier: "sb_SelectWOTypeForTemplateController") as? SelectWOTypeForTemplateController
        viewController?.params = workOrder
    ...
    func initViews() {
        ...
        timezoneField.setSelectedOption(option: "Central Time (UTC -5)")
        ...
        App.shared.api.getWorkOrderSetting() { (data, error) in
            ...
            self.workOrderNumField.text = "\(data?.work_order_num_prefix ?? "")\(data?.work_order_num_next.format5DigitNumber ?? "")"
        }
...
extension AddWorkOrderController: YPSignatureDelegate {
...
extension AddWorkOrderController: GMSAutocompleteViewControllerDelegate {
...
extension AddWorkOrderController: TLPhotosPickerViewControllerDelegate {
...
extension AddWorkOrderController: FSPagerViewDataSource, FSPagerViewDelegate {
