...
class QuickLinksController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func loadData() {
        // call api
        App.shared.api.getQuickLinks() { (list, error) in
            if let e = error {
    ...
    func processData(_ list: [QuickLink]) {
        // sort ungrouped
        let ungrouped = list.sorted(by: {$0.category > $1.category})
        // group
        let grouped = ungrouped.group(by: {$0.category})
        // sort
    ...
    func initNavBar() {
        ...
        // set title
        self.navigationItem.title = "Quick Links"
    }
...
extension QuickLinksController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let edit = UIContextualAction(style: .normal, title: "") {  (contextualAction, view, boolValue) in
            ...
            let viewController = self.storyboard?.instantiateViewController(withIdentifier: "sb_EditQuickLinkController") as? EditQuickLinkController
        ...
        if App.shared.user!.role.intValue <= 3 {
            ...
            let delete = UIContextualAction(style: .destructive, title: "") {  (contextualAction, view, boolValue) in
                ...
                App.shared.api.deleteQuickLink(item.id.intValue) { (success, error) in
...
extension QuickLinksController: FloatyDelegate {
    ...
    func initFAB() {
        ...
        self.pushTo(storyBoard: "Main", identifier: "sb_AddQuickLinkController")
            self.floaty.close()
        })
