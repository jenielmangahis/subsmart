...
class EmployeesController: UIViewController {
    ...
    func processData(_ list: [User]) {
        // sort ungrouped
        let ungrouped = list.sorted(by: {$0.FName.prefix(1) > $1.FName.prefix(1)})
        // group
        let grouped = ungrouped.group(by: {$0.FName.prefix(1)})
        // sort
    ...
    @objc func searchItem(_ textfield: UITextField) {
        ...
        filteredItems = items.filter({ item in
            return (item.FName.lowercased().contains(searchText.lowercased()) ||
                    item.LName.lowercased().contains(searchText.lowercased()) ||
                    item.email.lowercased().contains(searchText.lowercased()))
        })
...
extension EmployeesController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // avatar
        let avatar = Utils.createAvatar(28, 5, 40, "")
        cell.contentView.addSubview(avatar)
        
        // nameButton
        let nameButton = Utils.createPurpleButton(76, 12, Int(Device.width-40), item.FName + " " + item.LName, hasArrow: false)
        cell.contentView.addSubview(nameButton)
        
        // topLeft view
        let topLeft = Utils.createView(20, 50, Int(Device.width-40), 50, "EMAIL", item.email, [.top])
        cell.contentView.addSubview(topLeft)
        
        // chat icon
        let chatButton = UIButton(frame: CGRect(x: Device.width-42, y: 64, width: 22, height: 22))
        chatButton.setImage(UIImage.fontAwesomeIcon(name: .comment, style: .regular, textColor: .purple, size: CGSize(width: 24, height: 24)), for: .normal)
        cell.contentView.addSubview(chatButton)
        
        // bottomLeft view
        let bottomLeft = Utils.createView(20, 100, Int(Device.width-40), 50, "PHONE", item.phone.first?.number ?? "-", [.top])
        cell.contentView.addSubview(bottomLeft)
        
        // check phone
        //if let phone = item.phone.first?.number {
            // call icon
            let callButton = UIButton(frame: CGRect(x: Device.width-42, y: 114, width: 22, height: 22))
            callButton.setImage(UIImage.fontAwesomeIcon(name: .phone, style: .regular, textColor: .purple, size: CGSize(width: 24, height: 24)), for: .normal)
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let edit = UIContextualAction(style: .normal, title: "") {  (contextualAction, view, boolValue) in
            ...
            let viewController = self.storyboard?.instantiateViewController(withIdentifier: "sb_EditEmployeeController") as? EditEmployeeController
        ...
        if App.shared.user!.role.intValue <= 3 {
            ...
            let delete = UIContextualAction(style: .destructive, title: "") {  (contextualAction, view, boolValue) in
                ...
                let yesAction = UIAlertAction(title: "Yes", style: .default) { (alertAction) -> Void in
                    ...
                    App.shared.api.deleteUser(item.id.intValue) { (success, error) in
...
extension EmployeesController: FloatyDelegate {
    ...
    func initFAB() {
        ...
        floaty.addItem("Add", icon: UIImage.fontAwesomeIcon(name: .edit, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Others", identifier: "sb_AddEmployeeController")
            self.floaty.close()
        })
