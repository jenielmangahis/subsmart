...
extension EmployeesController: SideMenuNavigationControllerDelegate {
...
extension EmployeesController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // nameButton
        let nameButton = Utils.createPurpleButton(76, 12, Int(Device.width-40), "Jane Smith", hasArrow: false)
        cell.contentView.addSubview(nameButton)
        
        // topLeft view
        let topLeft = Utils.createView(20, 50, Int((Device.width-40)/2), 50, "EMAIL", "user@example.com", [.top, .right])
        cell.contentView.addSubview(topLeft)
        
        // topRight view
        let topRight = Utils.createView(Int(Device.width/2), 50, Int((Device.width-40)/2), 50, "PHONE", "(880) 888-8888", [.top])
...
extension EmployeesController: FloatyDelegate {
    ...
    func initFAB() {
        ...
        floaty.addItem("Add", icon: UIImage.fontAwesomeIcon(name: .edit, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_AddEmployeeController")
            self.floaty.close()
        })
