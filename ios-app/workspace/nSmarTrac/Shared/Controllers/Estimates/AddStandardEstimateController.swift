...
class AddStandardEstimateController: UITableViewController {
            ...
            if let id = result?.Data {
                ...
                App.shared.api.getEstimate(id.intValue, completion: { (item, error) in
                    ...
                    for image in self.selectedImages {
                        ...
                        App.shared.api.uploadEstimatePhoto(image, imageName: "\(item!.estimate_number)_photo_\(Date().currentTimeMillis()).jpg", params: params) { (success, error) in
                            SVProgressHUD.setDefaultMaskType(.none)
                            SVProgressHUD.dismiss()
                            guard error == nil else {
                                return SVProgressHUD.showError(withStatus: error?.localizedDescription ?? "")
    ...
    func configureViews() {
        ...
        App.shared.api.getEstimateSetting() { (data, error) in
            ...
            self.estimateNumField.text = "\(data?.estimate_num_prefix ?? "")\(data?.estimate_num_next.format5DigitNumber ?? "")"
...
extension AddStandardEstimateController: TLPhotosPickerViewControllerDelegate {
...
extension AddStandardEstimateController: FSPagerViewDataSource, FSPagerViewDelegate {
