...
class ContactsController: UIViewController {
    ...
    func loadData() {
        // call api
        App.shared.api.getContacts() { (list, error) in
            if let e = error {
    ...
    func processData(_ list: [Contact]) {
        // sort ungrouped
    ...
    @objc func searchItem(_ textfield: UITextField) {
        ...
        filteredItems = items.filter({ item in
            return (item.name.lowercased().contains(searchText.lowercased()) ||
                    ...
                    item.phone.lowercased().contains(searchText.lowercased()))
        })
...
extension ContactsController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // nameButton
        let nameButton = Utils.createPurpleButton(28, 12, Int(Device.width-40), item.name, hasArrow: true)
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let edit = UIContextualAction(style: .normal, title: "") {  (contextualAction, view, boolValue) in
            ...
            let viewController = self.storyboard?.instantiateViewController(withIdentifier: "sb_EditContactController") as? EditContactController
        ...
        if App.shared.user!.role.intValue <= 3 {
            ...
            let delete = UIContextualAction(style: .destructive, title: "") {  (contextualAction, view, boolValue) in
                ...
                let yesAction = UIAlertAction(title: "Yes", style: .default) { (alertAction) -> Void in
                    ...
                    App.shared.api.deleteContact(item.id.intValue) { (success, error) in
...
extension ContactsController: FloatyDelegate, UIGestureRecognizerDelegate {
    ...
    func initFAB() {
        ...
        floaty.addItem("Filter", icon: UIImage.fontAwesomeIcon(name: .filter, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_ContactFilterController")
            self.floaty.close()
        })
        floaty.addItem("Add", icon: UIImage.fontAwesomeIcon(name: .edit, style: .solid, textColor: AppTheme.defaultColor, size: CGSize(width: 30, height: 30)), handler: { item in
            self.pushTo(storyBoard: "Main", identifier: "sb_AddContactController")
            self.floaty.close()
        })
