//
//  APIClient+NotificationSetting.swift
//  nSmarTrac
//
...
extension APIClient {
    ...
    func getNotificationSetting(completion: @escaping (_ item: NotificationSetting?, _ error: NSError?) -> Void) {
        let path = apiURL.notificationSetting + "?company_id=\(App.shared.companyId)"
        
        APIClient.manager.request(path, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: getAuthHeaders())
            .validate()
            .responseObject { (response: DataResponse<NotificationSettingResponse>) in
    ...
    func putNotificationSetting(_ params: Parameters, completion: @escaping (_ success: Bool, _ error: NSError?) -> Void) {
        let path = apiURL.notificationSetting + "?company_id=\(App.shared.companyId)"
