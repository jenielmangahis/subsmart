//
//  APIClient+InvoiceSetting.swift
//  nSmarTrac
//
...
extension APIClient {
    ...
    func getInvoiceSetting(completion: @escaping (_ item: InvoiceSetting?, _ error: NSError?) -> Void) {
        let path = apiURL.invoiceSetting + "?company_id=\(App.shared.companyId)"
        
        APIClient.manager.request(path, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: getAuthHeaders())
            .validate()
            .responseObject { (response: DataResponse<InvoiceSettingResponse>) in
    ...
    func putInvoiceSetting(_ params: Parameters, completion: @escaping (_ success: Bool, _ error: NSError?) -> Void) {
        let path = apiURL.invoiceSetting + "?company_id=\(App.shared.companyId)"
