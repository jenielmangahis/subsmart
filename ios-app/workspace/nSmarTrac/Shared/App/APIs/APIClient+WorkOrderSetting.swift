//
//  APIClient+WorkOrderSetting.swift
//  nSmarTrac
//
...
extension APIClient {
    ...
    func getWorkOrderSetting(completion: @escaping (_ item: WorkOrderSetting?, _ error: NSError?) -> Void) {
        let path = apiURL.workOrderSetting + "?company_id=\(App.shared.companyId)"
        
        APIClient.manager.request(path, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: getAuthHeaders())
            .validate()
            .responseObject { (response: DataResponse<WorkOrderSettingResponse>) in
    ...
    func putWorkOrderSetting(_ params: Parameters, completion: @escaping (_ success: Bool, _ error: NSError?) -> Void) {
        let path = apiURL.workOrderSetting + "?company_id=\(App.shared.companyId)"
