//
//  APIClient+EstimateSetting.swift
//  nSmarTrac
//
...
extension APIClient {
    ...
    func getEstimateSetting(completion: @escaping (_ item: EstimateSetting?, _ error: NSError?) -> Void) {
        let path = apiURL.estimatesSetting + "?company_id=\(App.shared.companyId)"
        
        APIClient.manager.request(path, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: getAuthHeaders())
            .validate()
            .responseObject { (response: DataResponse<EstimateSettingResponse>) in
    ...
    func putEstimateSetting(_ params: Parameters, completion: @escaping (_ success: Bool, _ error: NSError?) -> Void) {
        let path = apiURL.estimatesSetting + "?company_id=\(App.shared.companyId)"
