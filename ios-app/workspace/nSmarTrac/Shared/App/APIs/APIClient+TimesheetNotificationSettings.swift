//
//  APIClient+TimesheetNotificationSettings.swift
//  nSmarTrac
//
...
extension APIClient {
    ...
    func getTimesheetNotificationSetting(completion: @escaping (_ item: TimesheetNotificationSetting?, _ error: NSError?) -> Void) {
        let path = apiURL.timesheetNotificationSettings + "?company_id=\(App.shared.companyId)"
        
        APIClient.manager.request(path, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: getAuthHeaders())
            .validate()
            .responseObject { (response: DataResponse<TimesheetNotificationSettingResponse>) in
    ...
    func putTimesheetNotificationSetting(_ params: Parameters, completion: @escaping (_ success: Bool, _ error: NSError?) -> Void) {
        let path = apiURL.timesheetNotificationSettings + "?company_id=\(App.shared.companyId)"
